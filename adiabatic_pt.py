# This is an Isothermal PT model
import numpy as np
import matplotlib.pyplot as plt
import P_iteration as P_i

V_c0 = 0.0001
V_cs = 0.1*V_c0
V_e0 = 0.00005
V_es = 0.1*V_e0
V_b0 = 0.00005
V_bs = 0.1*V_b0
phi = 30
pi = np.pi
f = 50
T_h = 300
T_c = 77
P_0 = 2*10**6

V_reg = 0.25*pi*0.045**2*0.065*0.7
V_hot = 0.1*V_reg
V_pul = 0.25*pi*0.024**2*0.1
V_cold = 0.1*V_pul
V_pon = 1.28
V_g0 = V_pul*V_pon
T_reg = (T_h - T_c)/np.log(T_h/T_c)
R = 2078.5
V_T = V_hot/T_h + V_reg/T_reg + V_cold/T_c
nt = 360
dt = 1/f/nt
nx = 26

n = np.linspace(1,nt,nt)
V_c = np.zeros(nt)
V_e = np.zeros(nt)
V_b = np.zeros(nt)
V_c[:] = V_cs + 0.5*V_c0*(1-np.cos(n[:]*pi/180-phi*pi/180))
V_e[:] = V_es + 0.5*V_e0*(1-np.cos(n[:]*pi/180))
V_b[:] = V_bs + 0.5*V_b0*(1+np.cos(n[:]*pi/180))

#--------------------
P = np.zeros((nt,nx))
P[:,0] = 1/((V_c+V_b)/T_h+V_T+(V_pul+V_e)/T_c)
P[:,1] = P[:,0]*P_0/np.mean(P[:,0])
for i in range(2,11,2):
    P[:,i],P[:,i+1] = P_i.P_iteration(V_c,V_b,V_T,V_pul,V_e,V_g0,P[:,i-1],P_0)

for i in range(12,17,2):
    P[:,i] = 1/((V_c+V_b)/T_h+V_T+(V_pul+V_e-V_g0*(0.5*(P[:,i-1]+P[:,i-3])/P_0)**-0.6)/T_c)
    P[:,i+1] = P[:,i]*P_0/np.mean(P[:,i])

for i in range(18,19,2):
    P[:,i] = 1/((V_c+V_b)/T_h+V_T+(V_pul+V_e-V_g0*(0.5*(P[:,i-1]+P[:,i-5])/P_0)**-0.6)/T_c)
    P[:,i+1] = P[:,i]*P_0/np.mean(P[:,i])

for i in range(20,23,2):
    P[:,i] = 1/((V_c+V_b)/T_h+V_T+(V_pul+V_e-V_g0*(0.5*(P[:,i-1]+P[:,i-3])/P_0)**-0.6)/T_c)
    P[:,i+1] = P[:,i]*P_0/np.mean(P[:,i])

for i in range(24,25,2):
    P[:,i] = 1/((V_c+V_b)/T_h+V_T+(V_pul+V_e-V_g0*(0.5*(P[:,i-1]+P[:,i-5])/P_0)**-0.6)/T_c)
    P[:,i+1] = P[:,i]*P_0/np.mean(P[:,i])

V_ge = np.zeros(nt)
V_ge = V_pul + V_e - V_g0*(P[:,25]/P_0)**-0.6
M_cold = P[:,25]*(V_c+V_ge)/(R*T_c)
M_reg = M_cold + P[:,25]*V_reg/(R*T_reg)
M_frc = np.zeros(nt)
M_frh = np.zeros(nt)
deltaV_c = np.zeros(nt)
deltaV_e = np.zeros(nt)
for i in range(nt):
    M_frc[i] = (M_cold[i]-M_cold[i-1])/dt
    M_frh[i] = (M_reg[i]-M_reg[i-1])/dt
    deltaV_c[i] = V_c[i]-V_c[i-1]
    deltaV_e[i] = V_e[i]-V_c[i-1]

PV_c_sum = sum(P[:,25]*deltaV_c)
PV_e_sum = sum(P[:,25]*deltaV_e)

plt.figure(1)
fig1 = plt.subplot()
fig1.plot(n,V_c,label = 'V_c')
fig1.plot(n,V_e,label = 'V_e')
fig1.plot(n,V_ge,label = 'V_ge')
fig1.set_ylabel('V')
fig1.set_xlabel('theta')
fig1.set_title('V-theta')
fig1.legend()

plt.figure(2)
ax1 = plt.subplot()
ax1.plot(n,M_frc,label = 'M_frc')
ax1.plot(n,M_frh,label = 'M_frh')
ax1.set_xlabel('theta')
ax1.set_ylabel('M[Kg/s]')
ax1.legend()
ax2 = ax1.twinx()
ax2.plot(n,P[:,25],'r-.',label = 'P25')
ax2.legend()
ax2.set_xlabel('theta')
ax2.set_ylabel('P[Pa]')
ax2.set_title('M/P-theta')
plt.tight_layout()

plt.figure(3)
fig3 = plt.subplot()
fig3.plot(V_c,P[:,25],label = 'V_c')
fig3.plot(V_e,P[:,25],label = 'V_e')
fig3.plot(V_ge,P[:,25],label = 'V_ge')
fig3.set_xlabel('v')
fig3.set_ylabel('P[Pa]')
fig3.set_title('P-v')
fig3.legend()

deltaP = P[:,25] - P[:,23]
fig = plt.figure(4)
ax1 = plt.subplot()
ax1.plot(n,deltaP,label = 'deltaP')
ax1.legend()
ax2 = ax1.twinx()
ax2.plot(n,V_ge,'r',label = 'V_ge')
ax2.legend()
plt.tight_layout()
plt.show()

data = open('data.txt','w')
data.write('V_cs '+str(V_cs)+'\n')
data.write('V_c0 '+str(V_c0)+'\n')
data.write('V_es '+str(V_es)+'\n')
data.write('V_e0 '+str(V_e0)+'\n')
data.write('V_bs '+str(V_bs)+'\n')
data.write('phi '+str(phi)+'\n')
data.write('PI '+str(pi)+'\n')
data.write('F '+str(f)+'\n')
data.write('T_h '+str(T_h)+'\n')
data.write('T_c '+str(T_c)+'\n')
data.write('V_cold '+str(V_cold)+'\n')
data.write('V_pul '+str(V_pul)+'\n')
data.write('V_g0 '+str(V_g0)+'\n')
data.write('T_reg '+str(T_reg)+'\n')
data.write('R '+str(R)+'\n')
data.write('V_pon '+str(V_pon)+'\n')
data.write('V_T '+str(V_T)+'\n')
data.write('dt '+str(dt)+'\n')

for line in P:
	print(*line,file=data)
data.close()
